...
- Gemini 2.5 Flash LLM (free tier via Google AI API)
- SQLite database queries
- Policy document retrieval

Design notes (changes from the starter template):
- Model is gemini-2.5-flash instead of gemini-2.5-pro, following the
  instructor's update about free-tier usage.
...
4. Keep the final answer short and factual, and base every claim on the tool
   output you received."""
    ...
    def query(self, user_query: str, user_role: str = "engineer") -> Dict[str, Any]:
        ...
        The reasoning loop:
        1. Send the question to Gemini with the system prompt and the
           declared tools.
        2. If Gemini returns function calls, execute each one and append the
           results to the conversation, then ask Gemini again.
        3. Repeat until Gemini returns a plain-text answer or MAX_TOOL_STEPS
           is reached.
        4. Track tokens and cost across every LLM call in the loop.

        Returns:
            Dict with keys: answer, tokens_used, cost, role
        """
